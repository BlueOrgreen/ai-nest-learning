...
describe('Gateway Proxy (e2e)', () => {
  ...
  describe('Proxy → user-service', () => {
    ...
    it('GET /api/users/:id  →  转发路径参数', async () => {
      ...
      await request(app.getHttpServer()).get('/api/users/uuid-1').expect(200);
    });
    ...
    it('PATCH /api/users/:id  →  转发 PATCH 方法', async () => {
      jest
        .spyOn(httpService, 'request')
        .mockReturnValueOnce(
          mockAxiosResponse(200, { id: 'uuid-1', name: 'Alice Updated' }),
        );
  ...
  describe('Proxy → order-service', () => {
    it('GET /api/orders  →  转发到 http://localhost:3002/orders', async () => {
      const mockOrders = [
        { id: 'order-1', userId: 'uuid-1', amount: 99.9, status: 'pending' },
      ];
      jest
        .spyOn(httpService, 'request')
  ...
  describe('4xx passthrough', () => {
    it('should pass 404 from downstream as-is', async () => {
      jest
        .spyOn(httpService, 'request')
        .mockReturnValueOnce(
          mockAxiosResponse(404, { message: 'User not found' }),
        );
    ...
    it('should pass 409 from downstream as-is', async () => {
      jest
        .spyOn(httpService, 'request')
        .mockReturnValueOnce(
          mockAxiosResponse(409, { message: 'Email already exists' }),
        );
