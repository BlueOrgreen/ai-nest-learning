...
export class OrdersController {
  ...
  @ApiQuery({
    name: 'productId',
    required: true,
    example: 'uuid-product-xxx',
    description: '商品 UUID',
  })
  @ApiResponse({
    status: 200,
    description: '脏读演示结果，含 uncommittedStock 字段',
  })
  @Get('demo/dirty-read')
  ...
  @ApiOperation({
    ...
    description:
      '修改 stock 后暂停 5 秒再 ROLLBACK，制造未提交的脏数据时间窗口。在此期间调用 GET /orders/demo/dirty-read 观察脏读。',
  })
  @ApiBody({
    schema: { example: { productId: 'uuid-product-xxx', dirtyStock: 0 } },
  })
  @ApiResponse({
    status: 201,
    description: '已触发脏写场景（后台等待 5 秒后回滚）',
  })
  ...
  @ApiOperation({
    ...
    description:
      '接口会等待 3 秒。在等待期间修改对应商品 stock 并提交，观察 firstRead 和 secondRead 是否不同。',
  })
  @ApiQuery({
    name: 'productId',
    required: true,
  ...
  @ApiResponse({
    status: 200,
    description: '含 firstRead / secondRead 对比的结果',
  })
  ...
  @ApiOperation({
    ...
    description:
      '接口会等待 3 秒。在等待期间插入该用户的新订单，观察 firstCount 和 secondCount 是否不同。',
  })
  @ApiQuery({
    name: 'userId',
    required: true,
  ...
  @ApiResponse({
    status: 200,
    description: '含 firstCount / secondCount 对比的结果',
  })
  ...
  @ApiOperation({
    ...
    description:
      'level 使用下划线格式，如 REPEATABLE_READ。接口内部会自动转为 SQL 标准格式（REPEATABLE READ）。',
  })
  ...
  @ApiResponse({
    status: 200,
    description: '含 firstRead / secondRead 及 isolationLevel 的结果',
  })
  ...
  @ApiOperation({
    ...
    description:
      '并发调用两次，两个请求都能立即获得共享锁（读读兼容），waitedMs 均接近 0。',
  })
  @ApiQuery({ name: 'productId', required: true, example: 'uuid-product-xxx' })
  @ApiResponse({
    status: 200,
    description: '含 stock / acquiredAt / waitedMs 的锁演示结果',
  })
  ...
  @ApiOperation({
    ...
    description:
      '并发调用两次，第二个请求被阻塞。观察返回的 waitedMs：第二个请求接近 3000ms。',
  })
  @ApiQuery({ name: 'productId', required: true, example: 'uuid-product-xxx' })
  @ApiResponse({
    status: 200,
  ...
  @ApiBody({
    schema: {
      example: { productIdA: 'uuid-product-a', productIdB: 'uuid-product-b' },
    },
  })
  @ApiResponse({
    status: 201,
    description: '死锁触发结果，含 winner / loser 信息',
  })
